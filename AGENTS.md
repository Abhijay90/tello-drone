...
## Key constraints
- Most scripts call `me.connect()` inline; `drone_control.py` uses the shared `DroneController` (`drone_controller.py`) instead.
...
- 2-file control pattern: `drone_controller.py` holds all drone movement (no input deps); input sources (gamepad/keyboard now, gestures later) map onto its API.
