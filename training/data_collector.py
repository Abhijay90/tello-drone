...
class WebcamDataCollector:
    ...
    def collect_sample(self, frame, landmarks):
        ...
        frame_path = sample_dir / f"{idx:04d}.png"
    ...
    def run(self):
        ...
        while True:
            ...
            # Always run hand detection to show user if hand is visible
            frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            mp_image = mp.Image(image_format=mp.ImageFormat.SRGB, data=frame_rgb)
            detection_result = self.mp_landmarker.detect(mp_image)
            h, w, _ = frame.shape

            has_hand = bool(detection_result.hand_landmarks and len(detection_result.hand_landmarks) > 0)

            # Draw detected landmark points so user can verify detection
            if has_hand:
            ...
            status_text = "HAND DETECTED" if has_hand else "NO HAND - show hand to camera"
            status_color = (0, 255, 0) if has_hand else (0, 150, 255)
            cv2.putText(frame, status_text, (w - 180, 30),
                        cv2.FONT_HERSHEY_PLAIN, 1.0, status_color, 2)

            # Auto-save gesture when window expires
            ...
            # Collect sample if capture is active and interval elapsed
            if self.current_gesture and (now - self._last_sample_time) >= self.CAPTURE_INTERVAL:
                if has_hand:
                    ...
                    self.collect_sample(frame, LandmarksWrapper(detection_result.hand_landmarks[0]))
                
                if not has_hand and self.current_gesture:
                    print("  [WARN] No hand detected during capture - show your hand to camera!")
    ...
    def run(self):
        ...
        while True:
            ...
            if self.mp_landmarker:
                ...
                # Draw detected landmark points
                if has_hand:
                    lm = detection_result.hand_landmarks[0]
                    for pt in lm:
                        cx, cy = int(pt.x * w2), int(pt.y * h)
                        cv2.circle(frame, (cx, cy), 3, (0, 255, 0), -1)

            # Status text
            status_text = "HAND DETECTED" if has_hand else "NO HAND - show hand to camera"
            status_color = (0, 255, 0) if has_hand else (0, 150, 255)
            cv2.putText(frame, status_text, (w2 - 180, 30),
                        cv2.FONT_HERSHEY_PLAIN, 1.0, status_color, 2)
                ...
                if has_hand:
                    class _LM:
                        def __init__(self, lm):
                            self.landmark = lm
                    self._collect_sample(frame, _LM(detection_result.hand_landmarks[0]))
                else:
                    print("  [WARN] No hand detected during capture!")
                self._last_sample_time = now
